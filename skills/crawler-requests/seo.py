...
def apply_xpath(html, xpath_map):
    ...
    for name, expr in xpath_map.items():
        ...
        # Если результат — число/булево (count(), boolean(), и т.д.)
        if isinstance(nodes, (int, float, bool)):
            result[name] = str(nodes)
            continue
        
        # Если результат — строка
        if isinstance(nodes, str):
            result[name] = nodes.strip()
            continue
        
        # Если результат — список узлов/строк
        vals = []
        for n in nodes:
